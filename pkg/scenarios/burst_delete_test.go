...
func TestRunBurstDelete(t *testing.T) {
	...
	// Make statefulset Get return ReadyReplicas == Spec.Replicas.
	...
	// Ensure namespace exists for PVC creation.
	...
	// Pre-create PVCs to match the label selector.
	...
	_, latencies, err := RunBurstDelete(ctx, client, config, 1*time.Millisecond)
	if err != nil {
		t.Fatalf("RunBurstDelete error: %v", err)
	}
